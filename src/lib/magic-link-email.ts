...
export function magicLinkEmail(locale: Locale, instanceName: string, link: string) {
  ...
  return {
    ...
    text: `${intro}\n\n${value.button}: ${link}\n\n${value.expires}\n${value.ignore}`,
    html: `<!doctype html><html lang="${locale}"><head><meta charset="utf-8"><meta name="viewport" content="width=device-width"></head><body style="margin:0;background:#f7f4ec;color:#12372a;font-family:Arial,sans-serif"><table role="presentation" width="100%" cellspacing="0" cellpadding="0" style="padding:32px 16px"><tr><td align="center"><table role="presentation" width="100%" cellspacing="0" cellpadding="0" style="max-width:560px;background:#fff;border:1px solid #d9ded8;border-radius:18px;padding:36px"><tr><td><p style="margin:0 0 8px;color:#6a776f;font-size:14px">${safeName}</p><h1 style="margin:0 0 20px;font-size:30px;line-height:1.15">${escapeHtml(value.subject)}</h1><p style="margin:0 0 26px;font-size:16px;line-height:1.6">${escapeHtml(intro)}</p><p style="margin:0 0 26px"><a href="${safeLink}" style="display:inline-block;background:#ef7d45;color:#102f26;text-decoration:none;font-weight:700;padding:15px 24px;border-radius:999px">${escapeHtml(value.button)}</a></p><p style="margin:0 0 8px;color:#5f6f66;font-size:14px;line-height:1.55">${escapeHtml(value.expires)}</p><p style="margin:0;color:#5f6f66;font-size:14px;line-height:1.55">${escapeHtml(value.ignore)}</p></td></tr></table></td></tr></table></body></html>`,
  };
}
