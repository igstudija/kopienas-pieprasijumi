...
export function LegalSettingsClient() {
  ...
  return (
    <section className="data-card legal-settings-card">
      <header className="settings-card-header"><div><h2>{copy.legalTitle}</h2><p>{copy.legalIntro}</p></div><button className="button button-dark button-small" type="button" onClick={() => setEditing(true)} disabled={loading || !settings}>{copy.edit}</button></header>
      ...
      {settings && <div className="settings-summary"><div><span>{copy.legalEntity}</span><strong>{settings.legalEntityName || copy.legalNotProvided}</strong></div><div><span>{copy.legalPrivacyContact}</span><strong>{settings.privacyContactEmail || settings.legalEmail || copy.legalNotProvidedMasculine}</strong></div><div><span>{copy.legalRetention}</span><strong>{settings.dataRetentionMonths} {copy.legalMonths}</strong></div><div className="settings-public-links"><Link href="/impressum">{copy.legalViewImprint}</Link><Link href="/privacy">{copy.legalViewPrivacy}</Link></div></div>}
      {editing && settings && (
